...
impl DesktopInput {
    ...
    pub fn on_key_pressed(&mut self, key: minifb::Key) {
        let map: Key = KEY_MAP.with(|f| f.borrow().get(&key).unwrap().clone());
        self.keys_down[map as usize] = true;
        self.keys_press[map as usize] = true;
    }

    pub fn on_key_released(&mut self, key: minifb::Key) {
        let map: Key = KEY_MAP.with(|f| f.borrow().get(&key).unwrap().clone());
        self.keys_up[map as usize] = true;
        self.keys_press[map as usize] = false;
    }
