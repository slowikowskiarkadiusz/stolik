...
const BLINKING_DELAY: f32 = 0.1;
const GARBAGE_INSERTION_DELAY: f32 = 4.0;
...
impl GarbageBar {
    ...
    pub fn add_lines(&mut self, count: u8) {
        ...
        if self.ready_timer.is_none() && !self.is_ready_to_be_popped {
            self.ready_timer = Some(GARBAGE_INSERTION_DELAY);
        }
    ...
    pub fn tick(&mut self, delta_time: f32) {
        if let Some(ref mut t) = self.ready_timer {
            *t -= delta_time;
            if *t <= 0.0 {
                self.is_ready_to_be_popped = true;
                self.ready_timer = None;
            }
        }
    ...
    pub fn render(&self) -> &ColorMatrix {
        if self.is_blink_on { &self.color_matrix } else { &self.blink_off_color_matrix }
    }
