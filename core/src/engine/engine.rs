...
use crate::{
    engine::{
        actor::actor::TActor, color::Color, color_matrix::ColorMatrix, input::input::Input, scene::Scene, threading_provider::Thread
    },
...
impl Engine {
    pub fn new(input: Box<dyn Input>) -> Self {
        let mut obj = Self {
            ...
            current_scene: RefCell::new(Box::new(PongScene::new())),
            input: RefCell::new(input),
        };

        obj.open_scene(Box::new(PongScene::new()));
    ...
    pub fn run<T: Thread>(
        ...
        33 - (last_timestamp as u64 - now_ms as u64)
    }
